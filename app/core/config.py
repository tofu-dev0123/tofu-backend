...
class Settings(BaseSettings):
    ...
    def database_url(self) -> str:
        ...
        url = f"mysql+pymysql://{self.db_user}:{password}@{self.db_host}:{self.db_port}/{self.db_name}?charset=utf8mb4&ssl=true"
        
        # デバッグ情報（本番環境でのトラブルシューティング用）
        # パスワードをマスクしたURLをログ出力
        masked_url = f"mysql+pymysql://{self.db_user}:***@{self.db_host}:{self.db_port}/{self.db_name}?charset=utf8mb4&ssl=true"
